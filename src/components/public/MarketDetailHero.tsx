...
function renderTitle(title: string, highlights?: string[]) {
  ...
  highlights.forEach((highlight, index) => {
    ...
    parts.push(
      <span key={`${highlight}-${index}`} className="text-emerald-300">
        {highlight}
      </span>
    )
...
export default function MarketDetailHero({
  ...
  const titleOnImage = banner?.titleVariant === 'dark'
        ...
        <FadeInStagger className="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-5" immediate stagger={0.12}>
          <FadeInItem className="lg:col-span-2 relative min-h-[280px] sm:min-h-[320px] rounded-xl overflow-hidden border border-gray-100 shadow-sm">
            ...
            <div
              className={
                titleOnImage
                  ? 'absolute inset-0 bg-gradient-to-t from-black/40 via-black/10 to-white/20'
                  : 'absolute inset-0 bg-gradient-to-t from-black/55 via-black/20 to-white/10'
              }
            />
            <div className="absolute inset-0 p-5 sm:p-7 flex flex-col justify-end">
              <h1
                className={`font-display font-bold text-xl sm:text-2xl lg:text-3xl leading-snug drop-shadow-sm max-w-2xl ${
                  titleOnImage ? 'text-graphite' : 'text-white'
                }`}
              >
